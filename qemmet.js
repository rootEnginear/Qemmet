...
function getMetadata(string) {
  ...
  const quantum_register =
    quantum_register_string.trim() === "" ? 1 : +quantum_register_string;
...
function getRegister(register_string) {
  const [single_digit, ...multiple_digits] = register_string
    .replace(/\s+/g, " ")
    .split(" ");
...
function getOpenQASMString(gate_string) {
  ...
  const qasm_string = gate_token_parsed
    .map(({ control_count, gate_name, gate_registers }) => {
      ...
      const gate_registers_all = gate_registers_normalized.map(
        (register) => register - 1
      );
  ...
  const qubit_declaration_string =
    quantum_register > 0 ? `qubit[${quantum_register}] qr;\n` : "";
  const bit_declaration_string =
    classical_register > 0 ? `bit[${classical_register}] cr;\n` : "";

  return `OPENQASM 3.0;
include "stdgates.qasm";
${
  quantum_register + classical_register ? "\n" : ""
}${qubit_declaration_string}${bit_declaration_string}
${qasm_string}`;
}
