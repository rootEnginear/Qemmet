...
const generateQubits = (qubit_count, depth) => {
    return new Array(qubit_count)
        .fill('')
        .map((_, i) => `<use xlink:href="#ket0" x="0" y="${i * 48}"></use><line x1="36" y1="${i * 48 + 16}" x2="${(depth + 1) * 40 + 8}" y2="${i * 48 + 16}" stroke="black" stroke-width="1" />`)
        .join('');
};
...
const generateMeasure = (qubit, qubit_count, column) => {
    return `<line x1="${40 * (column + 1) + 16 + 8 - 1.5}" y1="${48 * qubit + 16}" x2="${40 * (column + 1) + 16 + 8 - 1.5}" y2="${48 * (qubit + qubit_count) + 16 - 1.5 - 6}" stroke="black" stroke-width="1" /><line x1="${40 * (column + 1) + 16 + 8 + 1.5}" y1="${48 * qubit + 16}" x2="${40 * (column + 1) + 16 + 8 + 1.5}" y2="${48 * (qubit + qubit_count) + 16 - 1.5 - 6}" stroke="black" stroke-width="1" /><use xlink:href="#measure" x="${40 * (column + 1) + 8}" y="${48 * qubit}"></use><use xlink:href="#arrow" x="${40 * (column + 1) + 16 + 4 - 1}" y="${48 * (qubit + qubit_count) + 8 - 2}"></use>`;
};
const generateGate = (gate_name, gate_params) => {
    return (qubit, column) => {
        ...
        const params_str = gate_params
            ? `<text x="${40 * (column + 1) + 8 + 16 - 1}" y="${48 * qubit + 32 + 8 - 1}" dominant-baseline="middle" text-anchor="middle" font-family="LMR, LMM, 'Latin Modern Roman', 'Latin Modern Math', 'Computer Modern', serif" font-size='0.625rem'>(${formatted_params})</text>`
            : '';
        // No param
        return (`<use xlink:href="#gate" x="${40 * (column + 1) + 8}" y="${48 * qubit}"></use><text x="${40 * (column + 1) + 8 + 16 - 1}" y="${48 * qubit + 16 + 2}" dominant-baseline="middle" text-anchor="middle" font-family="LMRS, LMR, LMM, 'Latin Modern Roman', 'Latin Modern Math', 'Computer Modern', serif">${gate_name.toUpperCase()}</text>` +
            params_str);
    };
};
...
const generateVerticalLine = (qubits, column, isBarrier = false) => {
    const [max, min] = qubits.reduce(([max, min], qubit) => [Math.max(max, qubit), Math.min(min, qubit)], [-Infinity, Infinity]);
    return `<line x1="${40 * (column + 1) + 16 + 8}" y1="${min * 48 + 16}" x2="${40 * (column + 1) + 16 + 8}" y2="${max * 48 + 16}" stroke="black" stroke-width="1" ${isBarrier ? `stroke-dasharray="4"` : ''}/>`;
};
...
const separateMeasures = (gate_info) => {
    return gate_info
...
export const translateQemmetString = ({ qubit_count, bit_count, gate_info, }) => {
    ...
    const gates = normalized_gate_info
        ...
        if (gate_name === 'm')
            return generateMeasure(gate_registers[0], qubit_count, column);
        if (gate_name === 'b')
